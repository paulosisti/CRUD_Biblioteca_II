...
function inserir(dadosCliente, callback) {
    ...
    const values = [dadosCliente.nome, dadosCliente.telefone, dadosCliente.matricula];
...
function atualizar(dadosCliente, id, callback) {
    const cliente = new Client(connection);
    cliente.connect();
    const sql = 'UPDATE clientes SET nome_cliente = $1, telefone_cliente = $2, matricula = $3 WHERE id_cliente = $4 RETURNING *';
    const values = [dadosCliente.nome, dadosCliente.telefone, dadosCliente.matricula, id];
    cliente.query(sql, values, (err, res) => {
        if (err) {
            callback(err.message, undefined);
        }
