...
function inserir(emprestimo, callback) {
    validarEmprestimo(emprestimo.idCliente).then((resp) => {
        if (resp.ok == true) {
            ...
            const values = [emprestimo.idCliente, emprestimo.idLivro, dataEmprestimo, dataDevolucao];
...
function atualizar(emprestimo, idEmprestimo, callback) {
    const cliente = new Client(connection);
    cliente.connect();
    ...
    const values = [emprestimo.idCliente, emprestimo.idLivro, emprestimo.dataEmprestimo, emprestimo.dataDevolucao, idEmprestimo];
    cliente.query(sql, values, (err, res) => {
        if (err) {
            callback(err.message, undefined);
        }
