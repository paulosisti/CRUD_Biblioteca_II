...
function inserir(funcionario, callback) {
    ...
    const values = [funcionario.nome, funcionario.usuario, funcionario.senha];
...
function buscarPorUsername(username, callback) {
    const cliente = new Client(connection);
    cliente.connect();
    const sql = "SELECT * FROM funcionarios WHERE usuario = $1";
...
function atualizar(funcionario, id, callback) {
    ...
    const values = [funcionario.nome, funcionario.usuario, funcionario.senha, id];
    cliente.query(sql, values, (err, res) => {
        if (err) {
            callback(err.message, undefined);
        }
...
function login(id, funcionario, callback) {
    const cliente = new Client(connection);
    cliente.connect();
    const sql = "SELECT * FROM funcionarios WHERE id_funcionario = $1"
    const values = [id];
    cliente.query(sql, values, (err, res) => {
        if (err) {
            callback(err.name, undefined);
        } else if (Object.values(res.rows[0]).includes(funcionario.usuario && funcionario.senha)) {
            callback(undefined, "Usuário Validado!");
        } else {
            callback(`[ERRO] Verifique os dados digitados! 
Usuário:${funcionario.usuario} - Senha:${funcionario.senha}`, undefined);
        }
        cliente.end();
    })
}
...
module.exports = { listar, inserir, buscarPorNome, atualizar, deletar, login, buscarPorUsername }
