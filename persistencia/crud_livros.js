...
function inserir(livro, callback) {
    ...
    const values = [livro.isbn, livro.nome, livro.autor, livro.ano, livro.editora, livro.status];
...
function listar(callback) {
    const cliente = new Client(connection);
    cliente.connect();
    const sql = "SELECT * FROM livros ORDER BY id_livro";
...
function atualizar(livro, id, callback) {
    ...
    const values = [livro.isbn, livro.nome, livro.autor, livro.ano, livro.editora, livro.status, id];
    cliente.query(sql, values, (err, res) => {
        if (err) {
            callback(err.message, undefined);
        }
...
module.exports = { listar, inserir, buscarPorNome, buscarPorId, buscarPorStatus, atualizar, deletar }
